...
(function (global) {
    ...
    function venusPRadius(layout, y) {
        var rs = layout.radStraight;
        var rf = layout.radFlare;
        var yTop = layout.yStraightTop;
        var yBot = layout.yStraightBottom;
        var flareOut = Math.max(1, layout.pxFlareOut || FLARE_OUT_MM);
        var flareIn = Math.max(1, layout.pxFlareIn || FLARE_IN_MM);
        if (y <= yTop) {
            var u = Math.max(0, Math.min(1, (yTop - y) / flareOut));
            var s = smoothstep(u);
            return rs + (rf - rs) * (0.08 + 0.92 * s);
        }
        if (y >= yBot) {
    ...
    function projectStent(centerX, layout, theta, y) {
        var radius = venusPRadius(layout, y);
        return {
            x: centerX + Math.sin(theta) * radius,
            y: y - Math.cos(theta) * radius * STENT_TILT,
            depth: Math.cos(theta),
            theta: theta
        };
    }

    function stentNodeRows(layout) {
        var ys = [];
        ys.push(layout.yStraightTop);
        var bodyCells = Math.max(3, Math.round(layout.pxStentLen / Math.max(12, layout.radStraight * 0.5)));
        for (var i = 1; i < bodyCells; i++) {
            ys.push(layout.yStraightTop + (layout.yStraightBottom - layout.yStraightTop) * i / bodyCells);
        }
        ys.push(layout.yStraightBottom);
        ys.push(layout.yStraightBottom + layout.pxFlareIn * 0.48);
        ys.push(layout.yInflowBottom);
        return ys;
    }

    function collectStentStruts(centerX, layout) {
        var ys = stentNodeRows(layout);
        var n = STENT_CELLS;
        var struts = [];
        for (var r = 0; r < ys.length - 1; r++) {
            var off0 = (r % 2) * (Math.PI / n);
            var off1 = ((r + 1) % 2) * (Math.PI / n);
            for (var c = 0; c < n; c++) {
                var a0 = c * Math.PI * 2 / n + off0;
                var aRight = c * Math.PI * 2 / n + off1;
                var aLeft = (c - 1) * Math.PI * 2 / n + off1;
                struts.push({ a0: a0, y0: ys[r], a1: aRight, y1: ys[r + 1] });
                struts.push({ a0: a0, y0: ys[r], a1: aLeft, y1: ys[r + 1] });
            }
        }
        return struts;
    }

    function strokeVenusPStrut(ctx, centerX, layout, strut, style) {
        var steps = strut.crown ? 12 : 8;
        var midDepth = Math.cos(lerpAngle(strut.a0, strut.a1, 0.5));
        var t = (midDepth + 1) / 2;
        var alpha = style.backAlpha + (style.frontAlpha - style.backAlpha) * Math.pow(t, 1.25);
        var width = style.minW + (style.maxW - style.minW) * t;

        ctx.beginPath();
        for (var i = 0; i <= steps; i++) {
            var u = i / steps;
            var rise = strut.crown ? (1 - Math.pow(1 - u, 1.55)) : u;
            var p = projectStent(centerX, layout, lerpAngle(strut.a0, strut.a1, u), strut.y0 + (strut.y1 - strut.y0) * rise);
            if (i === 0) ctx.moveTo(p.x, p.y);
            else ctx.lineTo(p.x, p.y);
        }
        ctx.strokeStyle = 'rgba(32, 36, 42,' + alpha.toFixed(3) + ')';
        ctx.lineWidth = width;
        ctx.lineCap = 'round';
        ctx.lineJoin = 'round';
        ctx.stroke();
    ...
    function drawVenusPFrame(ctx, centerX, layout, style) {
        var struts = collectStentStruts(centerX, layout).concat(collectOutflowCrowns(layout));
        struts.sort(function (a, b) {
            return Math.cos(lerpAngle(a.a0, a.a1, 0.5)) - Math.cos(lerpAngle(b.a0, b.a1, 0.5));
        });
        ctx.save();
        ctx.setLineDash([]);
        for (var i = 0; i < struts.length; i++) {
            var depth = Math.cos(lerpAngle(struts[i].a0, struts[i].a1, 0.5));
            if (depth < -0.72) continue;
            strokeVenusPStrut(ctx, centerX, layout, struts[i], style);
        }
        ctx.restore();
    }

    function collectOutflowCrowns(layout) {
        var n = STENT_CELLS;
        var struts = [];
        var yJoin = layout.yStraightTop;
        var yTip = layout.yOutflowTop;
        for (var c = 0; c < n; c++) {
            var aTip = c * Math.PI * 2 / n;
            var aLeft = aTip - Math.PI / n;
            var aRight = aTip + Math.PI / n;
            struts.push({ a0: aLeft, y0: yJoin, a1: aTip, y1: yTip, crown: true });
            struts.push({ a0: aRight, y0: yJoin, a1: aTip, y1: yTip, crown: true });
        }
        return struts;
    }

    function drawRadiopaqueMarkers(ctx, centerX, layout) {
        var angles = [-0.95, 0, 0.95];
        var rows = [layout.yStraightTop, layout.yStraightBottom];
        var markerW = Math.max(3.2, layout.radStraight * 0.09);
        var markerH = Math.max(5.5, layout.radStraight * 0.16);
        for (var r = 0; r < rows.length; r++) {
            for (var m = 0; m < angles.length; m++) {
                var p = projectStent(centerX, layout, angles[m], rows[r]);
                var grd = ctx.createLinearGradient(p.x - markerW, p.y, p.x + markerW, p.y);
                grd.addColorStop(0, '#8A6410');
                grd.addColorStop(0.45, '#FFE27A');
                grd.addColorStop(1, '#A97812');
                ctx.fillStyle = grd;
                ctx.strokeStyle = '#6D5010';
                ctx.lineWidth = 0.7;
                ctx.beginPath();
                ctx.rect(p.x - markerW / 2, p.y - markerH / 2, markerW, markerH);
                ctx.fill();
                ctx.stroke();
            }
        }
    ...
    global.drawTechnicalStentMesh = function (ctx, centerX, layout, view) {
        ...
        drawVenusPFrame(ctx, centerX, layout, style);
        drawRadiopaqueMarkers(ctx, centerX, layout);
