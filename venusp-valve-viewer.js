...
(function (global) {
    ...
    function stentNodeRows(layout) {
        var ys = [];
        ys.push(layout.yOutflowTop);
        ys.push(layout.yOutflowTop + layout.pxFlareOut * 0.58);
        ys.push(layout.yStraightTop);
        var bodyCells = Math.max(3, Math.round(layout.pxStentLen / Math.max(11, layout.radStraight * 0.46)));
        ...
        ys.push(layout.yStraightBottom + layout.pxFlareIn * 0.50);
    ...
    function strokeVenusPStrut(ctx, centerX, layout, strut, style) {
        ...
        for (var i = 0; i <= steps; i++) {
            var u = i / steps;
            var p = projectStent(centerX, layout, lerpAngle(strut.a0, strut.a1, u), strut.y0 + (strut.y1 - strut.y0) * u);
    ...
    function drawVenusPFrame(ctx, centerX, layout, style) {
        var struts = collectStentStruts(centerX, layout);
